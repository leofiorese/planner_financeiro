...
export async function POST(request: Request) {
    ...
    `, [
            pid, plan.currentBalance || 0, plan.createdAt, plan.updatedAt,
            plan.currentBalance || 0, plan.updatedAt
        ]);
      ...
      `, [
                pid, fc.startingBalance || 0, fc.startDate, fc.months || 12, fc.includeGoalContributions ? 1 : 0, fc.conservativeMode ? 1 : 0, fc.updatedAt,
                fc.startingBalance || 0, fc.startDate, fc.months || 12, fc.includeGoalContributions ? 1 : 0, fc.conservativeMode ? 1 : 0, fc.updatedAt
            ]);
        }
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [item.id, pid, item.name, item.dueDay, item.closingDay, item.color || null, item.isActive ? 1 : 0, item.createdAt, item.updatedAt]);
        }
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [item.id, pid, item.name, item.amount, item.frequency, item.description || null, item.startDate, item.endDate || null, item.isActive ? 1 : 0, item.createdAt, item.updatedAt]);
        }
      ...
      `, [
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [item.id, pid, item.name, item.targetAmount, item.targetDate, item.currentAmount, item.description || null, item.category, item.priority, item.isActive ? 1 : 0, item.goalType, item.priorityOrder || 0, item.createdAt, item.updatedAt]);
        }
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [item.id, pid, item.month, item.projectedBalance, item.projectedIncome, item.projectedExpenses, item.projectedGoalContributions, item.startingBalance, item.netChange, item.generatedAt]);
        }
